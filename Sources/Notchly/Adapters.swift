...
struct CodexAdapter: ProviderAdapter {
    ...
    func fetch() async -> ProviderStatus {
        ...
        request.setValue("notchly", forHTTPHeaderField: "User-Agent")
        ...
        do {
            ...
            return ProviderStatus(provider: .codex, detected: true, source: "Codex OAuth", primary: usage.rateLimit?.primaryWindow?.window, secondary: usage.rateLimit?.secondaryWindow?.window, error: nil, updatedAt: Date(), costUsage: CodexCostCache.shared.current())
...
struct ClaudeAdapter: ProviderAdapter {
    ...
    private static func lookupToken() -> TokenLookup {
        // File credentials first: reading another app's Keychain item raises a
        // security dialog that hangs the refresh loop on dev builds. The file
        // is what Claude Code writes on this setup; Keychain stays as fallback.
...
private enum ProviderSecret {
    static func value(environmentNames: [String], keychainAccount: String) -> String? {
        ...
        for service in ["notchly", "nootch", "AgentNotch"] {
