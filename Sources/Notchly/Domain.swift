...
enum ProviderID: String, Codable, CaseIterable, Identifiable, Sendable {
    case codex, claude, openCode, openCodeGo, chatGPT, cursor, copilot, antigravity, xai, grok, groq, zai, clinePass, factory, gemini

    static let allCases: [Self] = [
        .codex, .claude, .gemini, .factory, .openCode, .chatGPT, .cursor, .copilot,
        .antigravity, .xai, .grok, .groq, .zai, .clinePass
    ]

    static let supported: [Self] = [
        .codex, .claude, .gemini, .factory, .openCode, .zai, .grok, .xai, .cursor, .copilot, .antigravity, .clinePass
    ]
    ...
    var icon: String {
        ...
        case .factory: "rectangle.and.pencil.and.ellipsis"
...
enum AppSettings {
    static let notchPositionKey = "notchly.notchPosition"
    static let notchPositionOffsetKey = "notchly.notchPositionOffset"
    static let animationDurationKey = "notchly.animationDuration"
    static let activityAnimationDurationKey = "notchly.activityAnimationDuration"
    static let overlayDisplayModeKey = "notchly.overlayDisplayMode"
    static let usageDisplayModeKey = "notchly.usageDisplayMode"
    static let showInDockKey = "notchly.showInDock"
    static let launchAtLoginKey = "notchly.launchAtLogin"
    static let providerIconShapeKey = "notchly.providerIconShape"
    static let themeColorKey = "notchly.themeColor"
    ...
    static func migrateLegacyPreferences(
        ...
        let migrationKey = "notchly.preferencesMigrated.v1"
        ...
        for domain in domains {
            for (key, value) in domain where key.hasPrefix("UsageNotch.") {
                let newKey = "notchly." + key.dropFirst("UsageNotch.".count)
    ...
    static let providerEnabledPrefix = "notchly.providerEnabled."
...
/// One signed-in subscription inside a provider that supports multiple
/// accounts (extra Claude logins, several Google AI Pro accounts).
struct AccountUsage: Sendable, Equatable, Codable {
    let label: String
    let primary: UsageWindow?
    let secondary: UsageWindow?

    var worstRemaining: Double? {
        let values = [primary?.remainingPercent, secondary?.remainingPercent].compactMap { $0 }
        return values.min()
    }
}
...
struct ProviderStatus: Identifiable, Sendable, Equatable, Codable {
    ...
    enum CodingKeys: String, CodingKey {
        case provider, detected, source, primary, secondary, error, updatedAt, costUsage, activity, accounts
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        self.init(
            ...
            accounts: try values.decodeIfPresent([AccountUsage].self, forKey: .accounts))
    }

    func withActivity(_ activity: AgentActivity) -> Self {
        Self(provider: provider, detected: detected, source: source, primary: primary, secondary: secondary, error: error, updatedAt: updatedAt, costUsage: costUsage, activity: activity, accounts: accounts)
    }
