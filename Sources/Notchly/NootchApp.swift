...
extension Notification.Name {
    static let openNotchlySettings = Notification.Name("Notchly.openSettings")
    static let settingsDidChange = Notification.Name("Notchly.settingsDidChange")
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var canvasSyncTask: Task<Void, Never>?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        applyDockVisibility()
        if let iconURL = ResourceBundle.url(forResource: "NotchlyIcon", withExtension: "png"),
        ...
        refreshTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                self?.store?.refresh()
                self?.panelController?.syncCanvasHeight(
                    providerCount: self?.store?.detectedStatuses.count ?? 0)
            }
        }
        // The canvas height is cheap to sync; do it promptly so the rail never
        // clips while statuses arrive.
        canvasSyncTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                self?.panelController?.syncCanvasHeight(
                    providerCount: self?.store?.detectedStatuses.count ?? 0)
                do {
...
struct SettingsView: View {
    ...
    var body: some View {
                ...
                SettingsSection(title: "Providers") {
                    ...
                    ForEach(Array(supported.enumerated()), id: \.element.id) { index, provider in
                        ...
                        VStack(spacing: 0) {
                            ...
                            ProviderExtrasRow(store: store, provider: provider)
        ...
        .alert("Restart required", isPresented: $restartRequired) {
            ...
            Text("Restart Notchly to apply the selected window style.")
        }
    ...
    private func updateLaunchAtLogin(_ enabled: Bool) {
        guard Bundle.main.bundleURL.pathExtension == "app" else {
            launchAtLoginError = "Launch at login is available after installing Notchly as an app."
            return
        }
        do {
            if enabled {
...
// MARK: - Multi-subscription connect controls

struct FactoryKeyField: View {
    let store: UsageStore
    @State private var key: String = FactoryAdapter.apiKey() ?? ""
    @State private var saved = false

    var body: some View {
        HStack(spacing: 8) {
            SecureField("Paste API key from app.factory.ai", text: $key)
            ...
            Button(saved ? "Saved" : "Save") {
                if key.trimmingCharacters(in: .whitespaces).isEmpty {
                    Keychain.delete(account: FactoryAdapter.keychainAccount)
                } else {
                    Keychain.set(key.trimmingCharacters(in: .whitespaces), account: FactoryAdapter.keychainAccount)
                }
                saved = true
                store.refresh()
...
struct AccountImportRow: View {
    enum Kind { case claude, gemini }
    let kind: Kind
    let store: UsageStore

    private var title: String {
        switch kind {
        case .claude: "Add Claude account…"
        case .gemini: "Add Google AI account…"
        }
    }
    private var message: String {
        switch kind {
        case .claude: "Pick the credentials.json of the Claude account to add"
        case .gemini: "Pick the oauth_creds.json of the Google AI account to add"
        }
    }
    private var defaultsKey: String {
        switch kind {
        case .claude: "notchly.claudeCredPaths"
        case .gemini: "notchly.geminiCredPaths"
        }
    }
...
/// Per-provider extras inside the Providers section: signed-in account rows for
/// multi-account providers plus the connect controls Notchly manages.
struct ProviderExtrasRow: View {
    let store: UsageStore
    let provider: ProviderID

    var body: some View {
        VStack(spacing: 0) {
            if let status = store.statuses.first(where: { $0.provider == provider }),
               let accounts = status.accounts, accounts.count > 1 {
                accountRows(accounts)
            }
            if provider == .factory {
                FactoryKeyField(store: store)
            }
            if provider == .claude {
                AccountImportRow(kind: .claude, store: store)
            }
            if provider == .gemini {
                AccountImportRow(kind: .gemini, store: store)
            }
        }
    }
            ...
            ForEach(Array(accounts.enumerated()), id: \.offset) { _, account in
                HStack(spacing: 8) {
                    ...
                    Text(account.label)
                        ...
                        .foregroundStyle(.secondary)
                    Spacer()
                    if let remaining = account.worstRemaining {
    ...
    private func tierDot(_ account: AccountUsage) -> Color {
        guard let remaining = account.worstRemaining else { return Color.white.opacity(0.4) }
        return UsageWindow(usedPercent: 100 - remaining).tierColor
    }
}
